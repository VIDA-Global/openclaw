...
/**
 * Installs a narrowly scoped global fetch wrapper for VIDA's OpenAI-compatible relay.
 *
 * This only injects headers for requests under `${VIDA_API_BASE_URL}/openai/v1`
 * and only when plugin hook execution has established an attribution scope.
 *
 * Keeping this at the fetch layer avoids patching plugins such as
 * memory-lancedb-pro, which instantiate their own OpenAI SDK clients.
 */
