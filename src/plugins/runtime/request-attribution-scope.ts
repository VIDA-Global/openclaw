...
/**
 * Runs plugin code with a request attribution scope that downstream HTTP clients can read.
 *
 * VIDA uses this to attribute plugin-owned OpenAI-compatible requests, such as
 * memory-lancedb-pro's internal chat/completions and embeddings calls, back to
 * the active OpenClaw agent/session without requiring a plugin fork.
 */
