...
const state = vi.hoisted(() => ({
  ...
  resolvedIsNewSessionMock: false,
}));
...
vi.mock("./command/session.js", () => ({
  clearRotatedSessionMetadata: (entry: SessionEntry) => ({
    ...entry,
  ...
  resolveSession: () => {
    ...
    return {
      ...
      isNewSession: state.resolvedIsNewSessionMock,
      persistedThinking:
...
describe("agentCommand – LiveSessionModelSwitchError retry", () => {
  beforeEach(() => {
    ...
    state.resolvedIsNewSessionMock = false;
  ...
  it("resets lifecycle metadata when early session writes rotate the session id", async () => {
    ...
    state.resolvedIsNewSessionMock = true;
