...
For a release with retained VIDA fork patches, create the branch first and do not tag until the
retained commits have been added and validated:
...
- leaves the branch exactly on that release unless you intentionally add fork-only commits
- pushes branch
- creates and pushes fork tag `vida-<tag>` (example: `vida-v2026.2.14`)
- runs downstream verifier (`scripts/verify-vida-release.sh`)

Use the default tag-pushing behavior only when the branch already contains the final intended fork
state, or when the target release intentionally has no retained fork commits.
